								...
								{getHeaders()
									...
									.filter(
										(header) =>
											activeTab === 1 ||
											(entityType === "school" &&
												["School Name", "UDISE Code", "Cluster Name", "Block Name"].includes(
													header
												)) ||
											(entityType === "student" &&
												["Student Name", "Enrollment ID", "Grade", "School ID"].includes(
													header
												))
									)
									.map((header) => (
							...
							{getSampleData().map((row, rowIndex) => (
								<TableRow key={rowIndex}>
									{getHeaders()
										...
										.map((header) => (
											<TableCell key={`${rowIndex}-${header}`} align="center">{row[header]}</TableCell>
										))}
								</TableRow>
							))}
						</TableBody>
