      ...
      <Box sx={{ px: 3, pt: 2, pb: 1 }}>
        ...
        <Box sx={{ mb: 1 }}>
          <Typography variant="body2" color="text.secondary" sx={{ display: "flex", alignItems: "center" }}>
            ...
            {searchTerm ? `Showing ${filteredErrors.length} of ${errorData.length} errors` : `Showing all ${errorData.length} errors`}
          </Typography>
        </Box>
      </Box>
                    ...
                    <TableRow key={`error-row-${index}`} hover>
                      {displayHeaders.map((header) => (
                        <TableCell key={`${index}-${header.id}`}>
                          {header.id === "class" ? (
                            error[header.id] ? `Class ${error[header.id]}` : ""
                          ) : header.id === "error" ? (
