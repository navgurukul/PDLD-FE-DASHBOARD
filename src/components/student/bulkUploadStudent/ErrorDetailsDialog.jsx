...
const StudentErrorDetailsDialog = ({ open, onClose, errorData, headers }) => {
	...
	const downloadErrorsCSV = () => {
		...
		link.setAttribute(
			"download",
			`student_error_rows_${loginDetails.name}_${new Date().toISOString().slice(0, 10)}.csv`
		);
			...
			<DialogContent dividers>
				<Box sx={{ mb: 2 }}>
					<Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
						{errorData.length} student record(s) could not be processed due to errors. Review and fix these
						issues.
					</Typography>
