...
const CSVMapper = ({ file, onMappingComplete, entityType = "student", apiResponse = null }) => {
  ...
  const getMappingStatus = () => {
    ...
    if (mappedFieldsCount === 0) {
      ...
      return { status: "incomplete", message: `${mappedFieldsCount} of ${totalRequiredFields} required fields mapped` };
    } else {
        ...
        <>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
            ...
            <Box>
              <Chip 
                icon={
                  mappingStatus.status === "complete" ? <CheckCircleIcon /> : 
                  mappingStatus.status === "incomplete" ? <InfoIcon /> : 
                  <ErrorIcon />
                }
                label={mappingStatus.message}
                color={
                  mappingStatus.status === "complete" ? "success" : 
                  mappingStatus.status === "incomplete" ? "info" : 
                  "error"
                }
                sx={{ mr: 2 }}
              />
                ...
                <Table size="small" stickyHeader>
                        ...
                        <TableCell key={`header-preview-${index}`}>
                          ...
                          {mappingConfig[header] && (
                            <Typography variant="caption" color="primary">
                              → {systemFields.find(f => f.value === mappingConfig[header])?.label || mappingConfig[header]}
                            </Typography>
                          )}
                        </TableCell>
                      ))}
                  ...
                  <TableBody>
                    {(isEditing ? editedData : csvData).slice(0, 20).map((row, rowIndex) => (
                      <TableRow 
                        key={`row-${rowIndex}`}
                        sx={rowIndex % 2 === 0 ? { backgroundColor: alpha('#f5f5f5', 0.3) } : {}}
                      >
            ...
            <Box>
              <Box sx={{ mb: 2 }}>
                <Alert severity="error">
                  <Typography variant="subtitle2">
                    There were {apiErrors.length} errors in your data upload. Please review the details below.
                  </Typography>
                </Alert>
              </Box>
                        ...
                        <Table size="small">
                          ...
                          <TableBody>
                            {Object.entries(selectedError.data).map(([key, value], idx) => (
                              <TableRow key={`detail-${idx}`} sx={idx % 2 === 0 ? { backgroundColor: alpha('#f5f5f5', 0.3) } : {}}>
                                <TableCell>
