...
/**
 * A beautiful and interactive pie chart component for displaying percentage-based data
 * with enhanced visual effects and animations
 * 
 * @param {Object} props - Component props
 * @param {number} props.percentage - The percentage value to display (0-100)
 * @param {string} props.primaryColor - The color for the primary segment (pass)
 * @param {string} props.secondaryColor - The color for the secondary segment (fail)
 * @param {boolean} props.animation - Whether to animate the chart on hover
 * @param {number} props.size - Size of the chart in pixels
 * @param {boolean} props.showAnimation - Whether to show loading animation on mount
    ...
    >
      ...
      {angle > 180 && (
        <div 
          className="absolute inset-0 rounded-full overflow-hidden transition-all duration-700"
          style={{ 
            clipPath: `polygon(50% 50%, 100% 0, ${50 + 50 * Math.sin((angle - 180) * Math.PI / 180)}% ${50 + 50 * Math.cos((angle - 180) * Math.PI / 180)}%, 50% 50%)`,
          }}
        >
          <div 
            className="absolute inset-0 rounded-full transition-all duration-700"
...
PieChart.defaultProps = {
  percentage: 0,
  primaryColor: '#24F24F', // Green theme to match bar chart
  secondaryColor: '#ffebee', // Light red
  animation: true,
